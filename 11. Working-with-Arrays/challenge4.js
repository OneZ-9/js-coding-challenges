...
const caclRecomandedFood = (weight) => {
  const recommendedFood = weight ** 0.75 * 28;

  return Math.trunc(recommendedFood);
};

// 1)
dogs.forEach((dog) => (dog.recommendedFood = caclRecomandedFood(dog.weight)));
console.log(dogs);

// 2)
const dogSarah = dogs.find((dog) => dog.owners.includes("Sarah"));

const checkEating = (curFood, recommendedFood) => {
  if (curFood > recommendedFood + recommendedFood * 0.1) {
    ...
    console.log(`Dog eating okay.`);
  }
};

checkEating(dogSarah.curFood, dogSarah.recommendedFood);

// 3)

const dogOwners = dogs.reduce(
  (acc, dog) => {
    if (dog.curFood > dog.recommendedFood + dog.recommendedFood * 0.1) {
      acc.ownersEatTooMuch.push(...dog.owners);
      //   acc.ownersEatTooMuch = [...acc.ownersEatTooMuch, ...dog.owners];
    } else if (dog.curFood < dog.recommendedFood - dog.recommendedFood * 0.1) {
      acc.ownersEatTooLittle.push(...dog.owners);
      //   acc.ownersEatTooLittle = [...acc.ownersEatTooLittle, ...dog.owners];
    }
    return acc;
  },
...
// 4)
console.log(
  `${dogOwners.ownersEatTooMuch.join(" and ")}'s  dogs eat too much!`
);
console.log(
  `${dogOwners.ownersEatTooLittle.join(" and ")}'s  dogs eat too little!`
);
...
// 7)
const dogsEatOkay = dogs.filter((dog) => eatOkay(dog));
